...
async function main() {
  ...
  if (!await evaluate(`(()=>{const r=document.querySelector('.node-plaque').getBoundingClientRect();return r.left>=7&&r.right<=innerWidth-7&&r.top>=7})()`)) throw new Error('La placa escalada queda fuera del tablero');

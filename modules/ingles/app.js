...
async function loadMaterial(translationMode) {
  const config = window.INGLES_CONFIG || {};
  const day = Number.parseInt(config.day, 10) || 0;
  const promptExtra = String(config.questionPrompt || '').trim().slice(0, 10000);
  try {
    ...
    if (translationMode) {
      ...
      renderCard(); showScreen('screen-tarjetas');
    } else {
      ...
      showScreen('screen-preguntas');
    }
  } catch (error) { console.error(error); openErrorOverlay('No se pudo procesar el material.', error); }
}
function loadTranslations() { return loadMaterial(true); }
function loadQuestions() { return loadMaterial(false); }

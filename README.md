...
## Widget de Síntesis por materia
...
`GET /api/inscreen/provider/synthesis?subjectId=…`,
el mismo endpoint con `weekNumber=…` para el documento y
`GET /api/inscreen/provider/synthesis-images?id=…` para sus imágenes.
Las semanas se obtienen de los objetos `synthesis-v2.json` existentes en R2;
las síntesis que todavía no se subieron no aparecen en el widget.
...
```powershell
# Reemplaza 2.0.20 por la versión nueva que acabas de configurar.
git switch main
git pull --ff-only origin main
git add mobile_android/app/build.gradle.kts
git commit -m "Release 2.0.20"
git push origin main
git tag -a v2.0.20 -m "Release 2.0.20"
